...
int scorePageTurn = 500; // time spent on each slide (in milliseconds)
...
void playGame(){
  ...
  if (beatSwitchState == 0 && lastBeatSwitchState == 1){ // when the switch goes from on to off
    ...
    else if (nowtime - pressedtime >= bpmupperlimit && pressureSwitchState == 0){
    ...
    //------ -1 when too fast
    else if (nowtime - pressedtime <= bpmlowerlimit && pressureSwitchState == 0){
    ...
    else if (pressureSwitchState == 0){ //if the bottom switch is not pressed
